<template>
  <v-container fluid ref="productListContainer">
    <v-row>
      <v-navigation-drawer
        fixed
      ...
      <v-col cols="12" lg="12">
        ...
        <div>
          <div
            v-if="!isLoading && !isTextLoading && productArray.length == 0"
            class="text-center system-text"
          >
        ...
        <div v-if="isTextLoading && !isLoading">
          <v-skeleton-loader
            v-for="index in 4"
            :key="index"
            type="list-item-avatar-three-line"
        ...
        ></div>
        <div
          v-if="productArray.length !== 0 && (finalPage || productArray.length % 20 !== 0)"
...
export default {
  ...
  computed: {
    ...
    // 新增：計算 v-intersect 的選項，用於提早觸發載入
    intersectOptions() {
      const halfHeight = this.productListHeight / 2;
      return {
        rootMargin: `${halfHeight}px 0px 0px 0px`,
      };
    },
  ...
  methods: {
    onIntersect(entries) {
      // 使用 v-intersect 傳入的 entries 來判斷元素是否可見
      const isIntersecting = entries[0].isIntersecting;
      
      // 確保元素進入視圖、沒有在載入中，並且還沒到最後一頁
      if (isIntersecting && !this.isLoading && !this.isTextLoading && !this.finalPage) {
        // 檢查當前頁碼是否小於總頁數
        if (this.page < this.pagingInfo.totalPageCount) {
          this.page++;
          // 手動觸發 watch 裡面的 page 邏輯
          this.toApply(false);
        } else {
          // 如果當前頁碼已經是總頁數，設定為最後一頁
          this.finalPage = true;
        }
      }
    },
    ...
    toApply(isNewFilter = true) {
      // 確保價格欄位是數字
      const maxPrice = parseInt(this.sortArrayTemplate.maxPrice);
      const minPrice = parseInt(this.sortArrayTemplate.minPrice);

      if (isNaN(maxPrice) || isNaN(minPrice)) {
          return; // 若不是有效數字，不執行後續操作
      }

      // 如果是新篩選，重置頁碼並將 finalPage 設為 false
      if (isNewFilter) {
          this.page = 1;
          this.finalPage = false;
      }
      
      // 確保 minPrice 不會大於 maxPrice
      if (maxPrice < minPrice) {
          [this.sortArrayTemplate.maxPrice, this.sortArrayTemplate.minPrice] = [this.sortArrayTemplate.minPrice, this.sortArrayTemplate.maxPrice];
      }
      ...
      this.$emit('updatedFilterConditions', this.filterConditions, isNewFilter)
      this.filterDrawer = false
    },
    // 新增：偵測商品列表容器高度的方法
    observeContainerHeight() {
      if (this.$refs.productListContainer) {
        this.productListHeight = this.$refs.productListContainer.clientHeight;
