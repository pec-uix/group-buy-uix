    ...
    return apiAxios({
      ...
      .then(res => {
        if (res.result) {
          ...
          this.color1 = res.result.color1 // 外框
          ...
          this.background_color1 = res.result.background_color1 || '#FFF9AA'
          this.text_color1 = res.result.text_color1 || '#DB073D'
          this.background_color2 = res.result.background_color2 || '#ed0186'
          this.text_color2 = res.result.text_color2 || '#ffffff'
          ...
          return Promise.all([
            apiAxios({
              ...
              this.gifts = res.result.map((x, index) => ({
                award_id: x.award_id,
                chance,
                text: x.award_wheel_name || x.award_name,
                textColor:
                  index % 2 === 0 ? this.text_color1 : this.text_color2,
                backgroundColor:
                  index % 2 === 0
                    ? this.background_color1
                    : this.background_color2,
    ...
    async triggerTurntable() {
      ...
      return apiAxios({
        ...
        .then(res => {
          ...
          setTimeout(() => {
            this.$dialog
              .show(`恭喜您轉中的獎項為${this.award_name}🎆`, {
                title: '訊息',
              })
              .then(() => {
                this.activityLogUpdated = true
          ...
          apiAxios({
            ...
            this.$store.commit('setActivityWheelCount', res.result)
          })
