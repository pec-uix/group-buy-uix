      ...
      <v-row>
        ...
        <v-col cols="12" md="9"
          ><v-card elevation="0">
            ...
            <v-row v-if="showDelSelect">
              <v-col cols="12" lg="12" class="d-flex justify-end pb-3 mt-4"
                ><v-btn dark class="primary mr-3" @click="delCheck">
                  刪除
                </v-btn>
            ...
            >
              <v-checkbox
                ...
                :style="`opacity:${
                  !(
                    !(recommend.recommend_status_name !== '許願中') &&
                ...
                "
              ></v-checkbox
              ><v-card class="mt-4 pa-3" height="100%" width="100%">
                <div class="d-flex">
                ...
                <div class="d-flex mt-2 align-center">
                  ...
                  <v-chip
                    ...
                    class="ml-2 mr-3"
                    :class="{
                      'white--text': recommend.push_delete_mark == 'N',
                    }"
...
export default {
  ...
  created() {
    ...
    if (this.url.searchParams.get('status')) {
      this.status = { profile_comments: this.url.searchParams.get('status') }
    }
  },
  data() {
    return {
  ...
  methods: {
    ...
    choseAll() {
      this.delItemArray = this.filteredRecommendArray
        .filter(
          item => item.recommend_status_name === '許願中' && !item.product_id
        )
        .map(item => item.id)
    },
    delCheck() {
      this.dialog = true
      this.delItemArray.forEach(item => {
        item
      })
    },
    delWishItem() {
      return apiAxios({
        method: 'post',
        url: '/Recommend/DeleteRecommend',
        data: { recommend_id: this.delItemArray },
      }).then(() => {
        this.fetchRecommend()
        this.showDelSelect = false
        this.delItemArray = []
      })
    },
  },
