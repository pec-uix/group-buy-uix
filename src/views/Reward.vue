        ...
        <div v-if="tab === 'tab-0'">
          ...
          <div class="d-flex justify-end mr-3 mb-2">
            <v-btn
              ...
              v-if="currentTab == 0 && readStatus == '未使用'"
            >
              <!-- :disabled="checkedItemLength == 0 || readStatus !== '未使用'" -->
              轉贈
            </v-btn>
            <v-btn
              class="ma-0 mr-3"
                  ...
                  <v-row>
                    ...
                    >
                      <v-btn
                        ...
                        :color="
                          item.text == readStatus
                            ? 'secondary lighten-1'
                            : '#444'
                        "
                        :outlined="item.text == readStatus ? false : true"
                  ...
                  >
                    <v-card-text class="pa-2 overflow-ellipsis">
                      ...
                      <div class="d-flex justify-start mt-2">
                        ...
                        > -->
                        <span v-if="currentTab == 0 && readStatus == '未使用'">
                          <v-btn
                            small
                            elevation="0"
                            rounded
...
export default {
  ...
  computed: {
    ...
    appBarHeight() {
      return document.querySelector('.v-app-bar').clientHeight
    },
  ...
  methods: {
    ...
    availableReward() {
      return apiAxios({
        method: 'post',
        url: '/Reward/GetAvailableReward',
      })
    },
    rewardCondition() {
      return apiAxios({
        method: 'post',
        url: '/Reward/GetRewardCondition',
      }).then(res => {
    ...
    insertReward(ticket) {
      ...
      return apiAxios({
        ...
        .then(res => {
          if (res.status == 1) {
            this.$snackbar.add(res.message, { color: 'success', timeout: 2000 })
          } else {
            this.$snackbar.add(res.message, { timeout: 2000 })
          }
        })
        .finally(() => {
    ...
    searchGiftedPhone() {
      return apiAxios({
        ...
        data: {
          phone_number: this.giftedPhone,
        },
      }).then(res => {
        if (res.message && !res.result.length) {
          this.$snackbar.add(res.message, {
            timeout: 2000,
          })
        } else {
          this.giftedPeople = res.result
          if (this.giftedPeople.length == 1) {
            this.selectGiftedPerson = this.giftedPeople[0].username
          }
          this.getRewardReceiver = res.result.length > 1 ? false : true
    ...
    giftReward() {
      if (this.$refs.form.validate()) {
        ...
        this.isLoading = true
        let data = {
          reward: this.giftDialogData.map(e => {
            // if (e.reward_status == 'receive') {
            //   return {
            //     reward_name: e.reward_name,
            //     amount_reached: e.amount_reached,
            //     reward_amount: e.amount,
            //     reward_id: e.reward_id,
            //     qty: e.gift_reward_num,
            //     status: e.reward_status,
            //     gifter: e.gifter_emp_no,
            //   }
            // }
            return {
              reward_name: e.reward_name,
          ...
          receive_emp_no: this.selectGiftedPerson,
        }
        return apiAxios({
          method: 'post',
          url: '/Reward/GiftReward',
          data: data,
        })
          .then(res => {
            // if (res.status) {
    ...
    sendPromoCode() {
      ...
      return apiAxios({
        method: 'post',
        url: '/Reward/ActivityCode',
        data: {
          activityCode: this.promoCode,
        },
      })
        .then(res => {
          this.promoStatus = res.result
          if (this.promoStatus.status == 'success') {
            this.promoStatus.title = '恭喜您成功領取折價券！'
            this.fetchReward()
          }
          if (this.promoStatus.status == 'error') {
            this.promoStatus.title = '折價券優惠碼無效'
          }
          if (this.promoStatus.status == 'claimed') {
            this.promoStatus.title = '已領取過'
          }
          if (this.promoStatus.status == 'finished') {
            this.promoStatus.title = '折價券優惠碼已領取完畢'
          }
