<template>
  <div style="max-width: 800px; margin: 0 auto">
    ...
    <template v-else>
      <v-alert v-if="award > 0" text type="success">
        答對，獲得OP點數 {{ award }}點
      </v-alert>
      <v-alert v-else-if="award === 0" text type="error"> 沒有答對 </v-alert>
      <v-btn
    ...
    >
      <v-img
        ...
        max-width="min(800px, 80vh)"
        :aspect-ratio="1 / 1"
        src="https://unilife.pec.com.tw/images/14338.jpg"
        @click="to"
      >
      </v-img>
    </v-dialog>
    ...
    return apiAxios({
      ...
      .then(res => {
        if (res.result) {
          ...
          this.router_to = res.result.router_to
  ...
  methods: {
    submit(answer_option_id) {
      return apiAxios({
        ...
        this.$dialog
          .show(
            res.result > 0 ? `答對，獲得OP點數 ${res.result}點` : `沒有答對`,
            {
              title: '訊息',
            }
          )
    ...
    to() {
      if (this.router_to.startsWith('/')) {
        this.$router.push(this.router_to)
      } else {
