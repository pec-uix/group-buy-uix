<template>
  <div>
    <div class="d-flex justify-end flex-wrap">
      ...
      <product-list
        :class-name="'ga-personalized-recommendation-list'"
        :section-name="'ga-personalized-recommendation-list'"
...
export default {
  ...
  created() {
    ...
    if (this.url.searchParams.get('type')) {
      this.type = this.url.searchParams.get('type')
    }
    if (this.url.searchParams.get('productId')) {
      this.productId = this.url.searchParams.get('productId')
    }
    if (this.type === 'home') {
      this.apiUrl = '/Product/GetProductRecommendPurchaseV3?type=home'
    } else if (this.type === 'product') {
      this.apiUrl = '/Product/GetProductRecommendPurchaseV3?type=product'
      this.apiData.productId = this.productId
    }
    this.fetchData()
  ...
  data() {
    return {
      ...
      type: '',
      productId: '',
      apiUrl: '/Product/GetProductRecommendPurchaseV3',
      apiParams: {
        pageNo: 1,
        pageSize: '20',
        totalPageSize: '200',
  ...
  methods: {
    ...
    updatedFilterConditions(filterConditions) {
      this.isLoading = true
      if (this.productId) {
        filterConditions.productId = this.productId
      }
      apiAxios({
        method: 'post',
        url: `${this.apiUrl}`,
